...
const { signinSchema, signupSchema, acceptCodeSchema } = require("../middlewares/validator");
...
exports.verifyVerificationCode= async(req, res) => {
	...
	try{
		...
		if(!existingUser.verificationCode || !existingUser.verificationCodeValidation){
			return res.status(400).json({success: false, message: "Code not sent"})
		}
		...
		if(hashedCodeValue===existingUser.verificationCode){
			...
			await existingUser.save()
			res.status(200).json({success: true, message: "your account has verified"})
		}
